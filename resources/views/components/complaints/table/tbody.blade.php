 <tbody class="divide-y divide-gray-50">
     <template x-for="complaintt in complaintts" :key="complaintt.id">
         <tr class="hover:bg-gray-50 transition cursor-pointer" @click="openDetails(complaintt)">
             <td class="px-4 py-3">
                 <div class="flex items-center gap-2.5">
                     ...
                     <span class="font-medium text-gray-700" x-text="complaintt.patient?.name || complaintt.patient_name">
                     </span>
                 </div>
             </td>
             ...
             <td class="px-4 py-3 text-gray-500 text-xs" x-text="complaintt.issue_type">
             </td>
             ...
             <td class="px-4 py-3 text-gray-500 text-xs" x-text="complaintt.resolution_notes">
             </td>
             <td class="px-4 py-3 text-gray-400 text-xs" x-text="formatDate(complaintt.visit_date)">
             </td>
             <td class="px-4 py-3 text-gray-400 text-xs" x-text="formatDate(complaintt.resolved_at)">
             </td>
             <td class="px-4 py-3">
                 <span class="text-xs px-2.5 py-1 rounded-full" x-text="formatDate(complaintt.created_at)"></span>
             </td>
             <td class="px-4 py-3">
                 <div class="flex items-center gap-3">
                     ...
                     <form :action="'{{ url('complaintts') }}/' + complaintt.id" method="POST">
                         @csrf
                         @method('DELETE')
