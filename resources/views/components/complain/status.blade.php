    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-3">
            <div class="w-10 h-10 rounded-lg bg-red-50 flex items-center justify-center flex-shrink-0">
                <i class="fa fa-flag text-red-500"></i>
            </div>
            <div>
                <p class="text-xs text-gray-400">إجمالي الشكاوى</p>
                <p class="text-xl font-medium text-gray-800">24</p>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-3">
            <div class="w-10 h-10 rounded-lg bg-amber-50 flex items-center justify-center flex-shrink-0">
                <i class="fa fa-circle-exclamation text-amber-500"></i>
            </div>
            <div>
                <p class="text-xs text-gray-400">قيد المراجعة</p>
                <p class="text-xl font-medium text-gray-800">8</p>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-3">
            <div class="w-10 h-10 rounded-lg bg-blue-50 flex items-center justify-center flex-shrink-0">
                <i class="fa fa-hourglass-half text-blue-500"></i>
            </div>
            <div>
                <p class="text-xs text-gray-400">في الانتظار</p>
                <p class="text-xl font-medium text-gray-800">5</p>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-3">
            <div class="w-10 h-10 rounded-lg bg-emerald-50 flex items-center justify-center flex-shrink-0">
                <i class="fa fa-circle-check text-emerald-500"></i>
            </div>
            <div>
                <p class="text-xs text-gray-400">تم الحل</p>
                <p class="text-xl font-medium text-gray-800">11</p>
            </div>
        </div>
    </div>
